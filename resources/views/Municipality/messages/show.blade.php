        ...
        <div class="col-lg-4">
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    <h3 class="card-title mb-0">Office Conversations</h3>
                    <a href="{{ route('municipality.requests.show', $activeRequest) }}" class="btn btn-outline-secondary btn-sm">Back To Request</a>
                </div>
            ...
            <div class="card shadow-sm" data-request-chat data-chat-style="bubbles" data-request-id="{{ $activeRequest->id }}" data-current-user-id="{{ Auth::id() }}">
                <div class="card-header border-0">
                    <div class="d-flex justify-content-between align-items-start">
                        <div>
                            <h3 class="card-title mb-1">{{ $activeRequest->user?->name ?? 'Citizen Conversation' }}</h3>
                            <div class="text-muted small">
                                {{ $activeRequest->service?->name ?? '-' }} · {{ $activeRequest->tracking_code }}
                            </div>
                        </div>
                        <a href="{{ route('municipality.requests.show', $activeRequest) }}" class="btn btn-primary btn-sm">Open Request Details</a>
                    </div>
                </div>
                <div class="card-body p-0">
                    <div class="px-3 py-2 border-bottom bg-light">
                        <span class="badge badge-light border">{{ $activeRequest->status }}</span>
                        <span class="text-muted small ml-2">{{ $office->name }}</span>
                    </div>
                ...
                <div class="card-footer bg-white">
                    <form method="POST" action="{{ route('municipality.requests.messages.store', $activeRequest) }}" enctype="multipart/form-data" data-chat-form>
                        ...
                        <div class="form-group mb-2">
                            <textarea name="body" rows="3" class="form-control @error('body') is-invalid @enderror" placeholder="Reply to the citizen...">{{ old('body') }}</textarea>
                            @error('body')
